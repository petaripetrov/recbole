...
class Trainer(AbstractTrainer):
    ...
    def popularity_compensation(self, scores, item_popularity, train_mask, alpha=1.0, beta=0.5):
        ...
        # User norm (n_u)
        uninteract = 1.0 - train_mask
        uninteract_scores = scores * uninteract.float()
        n_u = torch.norm(uninteract_scores, p=2, dim=1, keepdim=True)
...
class FAiRTrainer(Trainer):
    ...
    def _run_valid(self, data, verbose, show_progress):
        valid_start_time = time()
        valid_score, valid_result = self._valid_epoch(
    ...
    def fit(
        ...
        r"""Train the model based on the train data and the valid data. Modified to run 10 pretrain epochs 
            before continuing as recbole typically does
